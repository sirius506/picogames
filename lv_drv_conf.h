...
#define LV_DRV_INDEV_IRQ_READ    touch_read_irq()       /*Read the IRQ pin*/

/*---------
 *  SPI
 *---------*/
#define LV_DRV_INDEV_SPI_CS(val)            touch_cs(val)     /*Set the SPI's Chip select to 'val'*/
#define LV_DRV_INDEV_SPI_XCHG_BYTE(data)    touch_xchg_byte(data)      /*Write 'val' to SPI and give the read value*/
