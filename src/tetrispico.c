...
static unsigned int score,highscore; //得点、ハイスコア
unsigned int gcount=0; //カウンタ、乱数の種に使用
uint32_t keyold; //前回キー入力状態（リピート入力防止用）
...
static unsigned short sounddata[]={1911,1804,1703,1607,1517,1432,1351,1276,1204,1136,1073,1012,
...
static unsigned char startkeycheck(unsigned short n){
	// 60分のn秒ウェイト
	// スタートボタンが押されればすぐ戻る
...
static void sound(void);//プロトタイプ宣言
static void playmusic60thsec(void){
...
static void startmusic(unsigned char *m){
	music.p=m;
	music.startp=m;
	music.count=1;
	music.stop=0;
...
static void printchar(unsigned char x,unsigned char y,unsigned char c,unsigned char n){
