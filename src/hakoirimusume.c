...
// 駒の初期配置（数字は駒番号）
// 空き場所は負数
static const int8_t map[HAKOSIZEY][HAKOSIZEX]={
	{1,0,0,2},
	{1,0,0,2},
	{3,4,4,5},
	{3,6,7,5},
...
// グローバル変数定義
static _Block block[BLOCKNUM]; //駒の状態を保持
static int8_t hako[HAKOSIZEY][HAKOSIZEX]; //箱内の配置を保持
static int cursorx,cursory; //カーソル選択位置
static unsigned short step; //手数
static int8_t lastblock; //前回移動した駒番号
...
static void eraseblock(int8_t d){
//駒番号dの駒を消去
...
static void putspace(int x,int y){
